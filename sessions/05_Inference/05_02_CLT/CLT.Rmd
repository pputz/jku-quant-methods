...
Suppose we have have measurements from a randomly drawn sample (i.e. a random variable).
...
"In 95% of all cases there are 1.49 to 4.31 defects  
per 10,000 units."
...
for (n in c(1, 4, 10, 100)){
  ...
  h[[n]] <- ggplot(samples, aes(mean)) + 
    geom_histogram(fill="#FF9999", binwidth = 0.2) +
    scale_x_continuous(limits = c(1, 6.2), breaks=1:6) +
...
In 95% of all cases the actual population mean lays within the boundaries of the confidence interval.
...
In 95% of all cases the mean sepal length of  *iris setosa* is `r round(mn, 2)` ± `r round(error, 2)` cm.
